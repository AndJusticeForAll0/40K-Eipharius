...
//
// Chemical_produce_list acts as a reverse lookup table for chemical analysis
// For example:
// chemical_produce_list[/datum/chemical_reaction/inaprovaline] is a list of all possible recipes to produce inaprovaline.
// It is entirely possible to have multiple entries.
/proc/initialize_chemical_reactions()
	var/paths = typesof(/datum/chemical_reaction) - /datum/chemical_reaction
	chemical_reactions_list = list()
	chemical_produce_list = list()
	...
	for(var/path in paths)
		...
		if(D.result && D.result_amount > 0)
			var/reagent_id = D.result
			if(!chemical_produce_list[reagent_id])
				chemical_produce_list[reagent_id] = list()
			chemical_produce_list[reagent_id] += D
...
/datum/chemical_reaction/trueblood
	...
	var/blood_type = "O-"
	required_reagents = list(/datum/reagent/carbon = 1, /datum/reagent/iron = 1, /datum/reagent/blood = 3)
	result_amount = 5
